...
# Initialize chatbot as a global instance
...
class ChatRequest(BaseModel):
    message: str
    session_id: str = "default"  # Optional session ID for multi-user support

class ChatResponse(BaseModel):
    response: str
    ids: list
    api_processing_time: float = 0
    error: str = None
...
async def chatbot_endpoint(request: ChatRequest):
    ...
    try:
        ...
        return ChatResponse(
            response="I'm having trouble processing your fashion request right now. Please try again later.",
            ids=[],
            error=str(e),
...
if __name__ == "__main__":
    ...
    print(f"Starting Fashion Chatbot FastAPI server on port {port}...")
