...
function ProductsDisplay({
      ...
      try {
        ...
        const fetchProductDetails = async (id) => {
          try {
            ...
            // const res = await fetch(`http://localhost:10000/api/fetchproducts/${id}"`);
  ...
  const sendFilterData = async (filters) => {
    ...
    try {
      const response = await fetch("https://sproj-p14-code.onrender.com/api/filters", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleNextPage = () => {
    if (currentPage < totalPages) {
      ...
      fetch("https://sproj-p14-code.onrender.com/api/type-interest/pageClick", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
  ...
  const handleProductClick = (productId) => {
    const userId = localStorage.getItem("userId");
    const payload = { userId, blockId };
    fetch("https://sproj-p14-code.onrender.com/api/type-interest/productClick", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
  ...
  const logSearchQuery = async (query) => {
    ...
    try {
      await fetch("https://sproj-p14-code.onrender.com/api/searchhistory", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSearch = async () => {
    ...
    try {
      // Make a POST request to the correct backend API URL (localhost:10000)
      const response = await fetch('http://localhost:10000/api/searchProducts', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ query: searchTerm, skip: 0, limit: 10 }) // Adjust skip and limit based on your needs
      });
      ...
      if (response.ok) {
        ...
        // Log the products received from the API
        console.log('Products Received:', data.products);
  
        // Optionally, you can update your state to store the search results, if needed
        // setSearchResults(data.products);  // Uncomment this line to set products to state
      } else {
  ...
  const handleExternalLinkClick = (e, product) => {
    ...
    fetch("https://sproj-p14-code.onrender.com/api/trackProductView/productRedirected", {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
