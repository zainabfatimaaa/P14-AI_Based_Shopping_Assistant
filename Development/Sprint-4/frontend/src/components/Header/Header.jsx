...
import { getSlugFromCategoryName } from '../../utils/categorySlugUtils';
import './Header.css';
...
const Header = ({ setGenderFilter, setCategoryFilter }) => {
  ...
  // };
  const handleCategoryClick = (category) => {
    const genderSlug = selectedGender.toLowerCase();
    const categorySlug = getSlugFromCategoryName(category);
    navigate(`/${genderSlug}-${categorySlug}`);
    setShowDropdown(false);
  };
      ...
      <div className="menu-container" ref={dropdownRef}>
        ...
        {showDropdown && (
          <div className="dropdown-menu">
            {/* Gender Selection */}
            <div className="menu-logo" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span
                onClick={() => setShowDropdown(false)}
                style={{ cursor: 'pointer', fontSize: '50px', paddingLeft: '8px', marginBottom: '10px', fontWeight: 'lighter', color: 'black'}}
              >
                &times;
              </span>
              <h1 style={{ flex: 1, textAlign: 'center', marginRight: '34px' }}>Shop Savvy</h1>
            </div>
            ...
            <div className="subcategory-selection">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <span
                  className={selectedType === "Uppers" ? "selected" : ""}
                  onClick={() => handleTypeSelect("Uppers")}
                >
                  <div style={{ marginRight: '135px' }}> Uppers</div>

                </span>

                {selectedType === "Uppers" && (
                  <div className="category-list">
                    {categories[selectedGender]["Uppers"].map((category, index) => (
                      <div
                        key={index}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '20px' }}>
                <span
                  className={selectedType === "Bottoms" ? "selected" : ""}
                  onClick={() => handleTypeSelect("Bottoms")}
                >
                  <div style={{ marginRight: '135px' }}> Bottoms</div>
                </span>

                {selectedType === "Bottoms" && (
                  <div className="category-list">
                    {categories[selectedGender]["Bottoms"].map((category, index) => (
                      <div
                        key={index}
                        onClick={() => handleCategoryClick(category)}
                        className="dropdown-item"
